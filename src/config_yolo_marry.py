# Config for YOLO Marry & Hold (Continuous Buying, No Selling)
# Run with: python -m src.cli --config src.config_yolo_marry --yolo --marry --amount 0.005
...
# Trading parameters
BUY_AMOUNT: int | float = 0.005
BUY_SLIPPAGE: float = 0.25 # Default slippage for marry mode (adjust if needed)
SELL_SLIPPAGE: float = 0.02 # Ignored due to --marry

# Sell thresholds (Ignored due to --marry)
...
WAIT_TIME_AFTER_BUY: int | float = 120 # Ignored due to --marry
WAIT_TIME_BEFORE_NEW_TOKEN: int | float = 10 # Pause for YOLO mode

# Token and account management
MAX_TOKEN_AGE: int | float = 60
CLEANUP_MODE: str = "disabled" # Consider "post_session" if running long YOLO sessions
