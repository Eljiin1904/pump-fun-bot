...
from ..core.pubkeys import LAMPORTS_PER_SOL, TOKEN_DECIMALS, PumpAddresses, SystemAddresses
...
class TokenSeller(Trader):
    ...
    async def execute(self, token_info: TokenInfo, *args, **kwargs) -> TradeResult:
        """Execute sell operation for a given token."""
        try:
    ...
    async def _send_sell_transaction(
        ...
        try:
            return await self.client.build_and_send_transaction(
                ...
                priority_fee=await self.priority_fee_manager.calculate_priority_fee(self._get_relevant_accounts(token_info)),
            )
        except Exception as e:
